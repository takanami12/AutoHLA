...
def map_diploid_pairs(prob: np.ndarray, topk: int = 5, tau: float = HW_TAU):
    ...
    if prob.ndim != 2 or not prob.shape[1] or not np.isfinite(prob).all():
        raise ValueError("prob must be a finite nonempty allele matrix")
    if (prob < 0).any() or not np.allclose(prob.sum(1), 1.0):
        raise ValueError("prob rows must be nonnegative and sum to one")
    if not np.isfinite(tau) or tau <= 0 or topk < 1:
        raise ValueError("tau must be finite and positive; topk must be positive")
    # Only two candidates can win: (largest, largest) or (largest, second).
    # Keep argsort's tie order, matching the historical exhaustive decoder.
    top = np.argsort(-prob, axis=1)[:, :min(topk, 2)]
    first = top[:, 0]
    second = top[:, -1]
    rows = np.arange(len(prob))
    hom = prob[rows, first] ** 2
    het = tau * prob[rows, first] * prob[rows, second]
    choose_het = (first != second) & (het > hom)
    pairs = np.column_stack((first, np.where(choose_het, second, first)))
    squared = np.square(prob).sum(1)
    normalizer = squared + tau * (prob.sum(1) ** 2 - squared) / 2.0
    posterior = np.where(choose_het, het, hom) / normalizer
...
def tune_tau(blocks, grid=TAU_GRID, topk: int = 5, *, prefer_hw=False) -> float:
    ...
    Product uses prefer_hw=True: ties prefer tau closest to Hardy-Weinberg.
    Legacy callers retain grid order. Incomplete labels are excluded; unseen
    valid copies remain false negatives when valid-copy counts are supplied.
    """
    blocks = list(blocks)
    if prefer_hw:
        grid = sorted(set(grid) | {HW_TAU}, key=lambda t: (abs(np.log(t / HW_TAU)), t))
    best = (-1.0, HW_TAU)
    for tau in grid:
        tp = pred_sum = truth_sum = 0.0
        for block in blocks:
            prob, truth = block[:2]
            valid = np.asarray(block[2]) if len(block) > 2 else truth.sum(1)
            # Incomplete labels cannot identify both copies; unseen valid copies
            # remain in the denominator when the caller supplies valid counts.
            keep = valid == 2
            prob, truth, valid = prob[keep], truth[keep], valid[keep]
