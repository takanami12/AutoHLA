...
def _blended_probs(net, score, z, beta, ridge, af_split, pair=None, x=None) -> dict:
    ...
    `freq` lay tu `prob.mean(0)` chu khong tu nhan: luc impute khong co nhan, va
    fit tau tren mot uoc luong AF khac se lech dung o cho beta doi nhanh.
    """
    base = score
    if pair is not None:
        if x is None:
    ...
    for gene, sl, _ in _gene_slices(net.outputs_size):
        ...
        if beta is not None and ridge is not None and gene in ridge:
            ...
            prob = blend(prob, extra, beta[gene][0], beta[gene][1], prob.mean(0),
                         af_split=af_split)
        out[gene] = prob
    return out
...
def _pair_dosage(net, cfg, parts, epochs, log=print):
    ...
    pair_net, head, _ = train_pair(
        ...
        torch.as_tensor(parts["train"][2], dtype=torch.float32), parts["train"][2],
