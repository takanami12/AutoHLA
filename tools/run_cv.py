...
Cai duong nghien cuu cu (mot cohort/*.sh + mot dong bien moi truong
...
lap duoc so, do la loi cua goi chu khong phai cua mot lop vá rieng.
    ...
    python run_cv --cohort vn1k --folds 1-10 --groups 1-4 \
        --out do noi bo --par 32 --threads 4
...
# `tools/` nam trong AutoHLA/ o cay noi bo va o GOC cay public. Do do KHONG duoc
# dem so cap thu muc: PKG la thu muc chua goi `autohla/` (cwd cua subprocess), con
# ROOT la thu muc chua du lieu. Ban goc dung parents[2] nen cay public phai sua tay,
# va do la mot nguon troi giua hai cay.
PKG = next(p for p in Path(__file__).resolve().parents
           if (p / "autohla" / "__init__.py").exists())
ROOT = next(p for p in (PKG, *PKG.parents)
            if (p / "data" / "references").is_dir())
COHORTS = {
...
def cell_dir(out_root, cell: Cell) -> Path:
    # resolve: `run_cell` goi subprocess voi cwd=PKG, nen mot --out tuong
    # doi lam model/ va calls.csv roi vao AutoHLA/<out>, con `done` thi tim o
    # <out> so voi CWD cua tien trinh cha -- khong bao gio thay, nen o nao cung
    # bi chay lai tu dau.
    return Path(out_root).resolve() / cell.arm / cell.name


def plan_cells(folds, groups, cohort="vn1k"):
    return [Cell(cohort, int(f), int(g))
