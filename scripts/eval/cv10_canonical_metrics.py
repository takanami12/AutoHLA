...
- micro theo AF bin, TP = min(truth, dosage); `sn` micro, `ppv`/`f1` trung bình
  theo allele có trọng số = số bản sao thật; `concordance` trên hard call.
    ...
    python cv10_canonical_metrics pool     # gộp AEHLA_PAIR
    python cv10_canonical_metrics score
"""
...
# Chi mot thu muc: `common` va `compute_raw_chip_maf_metrics` deu nam o day.
# Hai entry cu (mot cohort, ban goc) da chet -- do duoc bang cach import lai
# voi mot minh duong nay.
...
# tên hiển thị -> file gộp trong do noi bo cohort
METHODS = {
    "SNP2HLA": "SNP2HLA",
    "CookHLA": "CookHLA",
    "DEEP*HLA": "DeepHLA",
