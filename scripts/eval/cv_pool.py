...
1. `pool`  — gộp dự đoán test của 10 fold thành MỘT file duy nhất
             `results/cv_vn1k/predictions/<method>.dosage.csv.gz`.
...
Dùng:
    python scripts/eval/cv_pool.py pool --method SNP2HLA CookHLA
    python scripts/eval/cv_pool.py score
"""
import argparse
import os
...
# Cùng bảng màu với `scripts/eval/plot_raw_chip_maf_metrics.py` để hai hình đọc chung được.
...
def plot(summary):
    """Figure1: cùng bố cục với `results/metrics/raw_chip_maf/methods_by_maf_bin.png`
